...
interface DateRangeGroup {
  ...
  startYear: number;
  endYear: number;
  startDate: string; // YYYY/M/D format (no leading zeros)
  endDate: string;   // YYYY/M/D format (no leading zeros)
  startRow: number;  // Google Sheets row number (1-indexed)
  endRow: number;     // Google Sheets row number (1-indexed)
...
const generateDateRanges = (): DateRangeGroup[] => {
...
export { generateDateRanges, type DateRangeGroup };
