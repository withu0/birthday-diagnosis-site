...
export async function POST(request: NextRequest) {
  try {
    ...
    const basicResponse = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/api/judge/basic`, {
      method: 'POST',
      headers: {
    ...
    const talentResponse = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/api/judge/talent`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
